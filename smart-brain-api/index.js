...
// }); app.get('/profile/:id', (req, res) => {   const { id } = req.params; let
// found = false;   dataBase.users.forEach(user => {    if (user.id === id) {
...
// === dataBase.users[0].password   ) { res.json("success");   } else {
// res.status(400).json("Error logging in");   } }); app.post('/image', (req,
// res) => {   const { id } = req.body;   let found = false;
// dataBase.users.forEach(user => {     if (user.id === id) { found = true;
//  user.entries++;       return res.json(user.entries); }   });  if (!found) {
//    res.status(404).json("No such user");   } }); app.post('/register', (req,
// res) => {   const { email, name, password } = req.body;
// dataBase.users.push({     id: '125',     name:name,     email: email,
// password: password,     entries: 0,     joined: new Date()   });
